...
public class UserAcceptanceTest extends AcceptanceTest {
    ...
    public void create() throws Exception {
        ...
        postBuilder.addParam("name", "자바지기");
        postBuilder.addParam("email", "javajigi@slipp.net");

        ResponseEntity<String> response = template().postForEntity("/users", postBuilder.build(), String.class);
    ...
    private ResponseEntity<String> update(TestRestTemplate template) throws Exception {
        PostRequestBuilder postBuilder = PostRequestBuilder.urlEncodedHeader();
        postBuilder.addParam("_method", "put");
        postBuilder.addParam("password", "password2");
        postBuilder.addParam("name", "자바지기2");
        postBuilder.addParam("email", "javajigi@slipp.net");

        return template.postForEntity(String.format("/users/%d", defaultUser().getId()), postBuilder.build(), String.class);
    }
